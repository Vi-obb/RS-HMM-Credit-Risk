...
The empirical design centres on three logistic specifications: a borrower-only baseline, a macro-augmented baseline, and a regime-aware model. The comparison is intentionally hierarchical: each core model adds one source of information while preserving a common loan-month sample, label definition, and chronological evaluation design. The current robustness pass also reports a lagged-stress regime-aware variant, which tests whether dynamic stress information changes the result without replacing the core three-model thesis comparison.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{L{0.22\textwidth}L{0.34\textwidth}L{0.29\textwidth}}
        ...
        Lagged regime-aware logit & Borrower features, filtered stress probability, lagged stress variables, stress change, stress duration, and stress interactions. & Robustness check for dynamic regime effects. \\
        \bottomrule
    \end{tabular}
\end{table}
...
The lagged-stress variant augments Equation~\ref{eq:regime-aware-model} with stress dynamics:
\[
    \hat p_{t-3}^{\mathrm{Stress}},\quad
    \hat p_{t-6}^{\mathrm{Stress}},\quad
    \hat p_{t-12}^{\mathrm{Stress}},\quad
    \Delta \hat p_t^{\mathrm{Stress}},\quad
    D_t^{\mathrm{Stress}},
\]
where $\Delta \hat p_t^{\mathrm{Stress}}=\hat p_t^{\mathrm{Stress}}-\hat p_{t-1}^{\mathrm{Stress}}$ and $D_t^{\mathrm{Stress}}$ is the current run length of hard-stress months. This variant is treated as a robustness specification because it increases model complexity and is not the primary thesis comparison.
...
\end{equation}
The current empirical pass estimates the HMM on the mortgage study window, with a 12-month lookback included so lagged stress features can be constructed. Filtered probabilities are then generated recursively so that the stress probability at month $t$ depends on macro observations observed through $t$, not on future macro observations. This is a methodological improvement over using full-sequence smoothed probabilities from the entire FRED history. A stricter fully real-time extension would re-estimate the HMM on an expanding or rolling window; this is left as a robustness direction rather than claimed as the current implementation. The downstream PD models are fit on $\mathcal{R}_{\mathrm{train}}$, tuned on $\mathcal{R}_{\mathrm{valid}}$, calibrated on validation predictions, and evaluated once on $\mathcal{R}_{\mathrm{test}}$.
